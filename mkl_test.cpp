...
#include <stdio.h>
#include <stdlib.h>
#include "/util/intel/cmkl/8.0.1/include/mkl.h"
extern "C" double ddot_(int *N, double *dx, int *incx, double *dy, int *incy);//prototypes mkl ddot function
int main() {
	for (int n = 1; n<6; n++){//master loop to change the size of vectors to test
		...
		int inc2 = inc1;
		clock_t totalt1 = 0; //initialize total time to zero
		...
		printf("Vectors with %i elements took an average time of %.6f seconds to multiply.\n", N, averaget1);
	}
}